      ...
      >
        <div class="md:[direction:ltr]">
          <div class="relative overflow-hidden rounded-2xl aspect-video bg-dark-100 cursor-pointer group" @click="openLightbox(project, 0)">
            <img :src="project.images[0]" :alt="`${project.title} screenshot`" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
            <div class="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
          </div>
          <div v-if="project.images.length > 1" class="grid grid-cols-3 gap-3 mt-3">
            <div v-for="(img, i) in project.images.slice(1, 4)" :key="i" class="aspect-video rounded-xl overflow-hidden bg-dark-100 cursor-pointer group" @click="openLightbox(project, i + 1)">
              <img :src="img" :alt="`${project.title} screenshot ${i + 2}`" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
            </div>
          </div>
        </div>
        ...
        <div class="md:[direction:ltr] space-y-4">
          ...
          <div class="flex flex-wrap gap-2 pt-2">
            <span v-for="tech in project.technologies" :key="tech" class="px-3 py-1 text-xs font-medium rounded-lg bg-dark-100 text-dark-600">{{ tech }}</span>
          </div>
        </div>
      </article>
    </div>
