...
<template>
  <div ref="pageRef" class="bg-cream-50 dark:bg-charcoal-900">
    <!-- ───────── MASTHEAD ───────── -->
    <header class="section-container pb-16 pt-28 lg:pb-20">
      <p data-reveal class="mb-4 text-xs font-bold uppercase tracking-[0.3em] text-vermillion">
        {{ t("projects.title") }}
      </p>
      <h1 data-reveal class="section-title">{{ t("projects.ecosystemTitle") }}</h1>
      ...
      <div class="grid gap-8 lg:grid-cols-2">
        ...
        >
          <div class="grid grid-cols-2 gap-px bg-charcoal-100 dark:bg-charcoal-700">
            <button
              v-for="(img, idx) in proj.images.slice(0, 2)"
              :key="idx"
              class="group/img relative overflow-hidden bg-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-vermillion dark:bg-charcoal-800"
              @click="openGallery(proj.images, idx, $event)"
            >
              <img
                :src="img"
                :alt="`${t(`projects.items.${proj.id}.title`)} ${idx + 1}`"
          ...
          <div class="flex flex-1 flex-col p-6 lg:p-8">
            ...
            <div class="mt-auto pt-6">
              ...
              <a
                ...
                class="mt-5 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-vermillion transition-colors hover:text-vermillion-700"
              >
      ...
      <div class="section-container">
        <div class="grid items-center gap-10 lg:grid-cols-12 lg:gap-12">
          <div class="lg:col-span-5">
            ...
            <router-link
              data-reveal
              to="/projects/movize"
              class="mt-6 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-vermillion transition-colors hover:text-vermillion-700"
            >
              {{ t("projects.readCaseStudy") }}
              <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
          ...
          <div data-reveal class="lg:col-span-7">
            <button
              class="group block w-full overflow-hidden border border-charcoal-100 focus:outline-none focus:ring-2 focus:ring-vermillion dark:border-charcoal-700"
        ...
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <button
              v-if="mod.image"
              class="block overflow-hidden focus:outline-none focus:ring-2 focus:ring-inset focus:ring-vermillion"
